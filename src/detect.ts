 ...
 */
function indexOfSequence(haystack, needle, startIndex = 0) {
 ...
 */
function hasValidPngIEND(trailer) {
...
function parsePng(pngData) {
    let uint8Array = new Uint8Array(pngData);

    if (!uint8Array.slice(0, PNG_MAGIC.length).every((v, i) => v == PNG_MAGIC[i])) throw new Error("Not a PNG file!");
    ...
    return trailer.byteLength > 0 && hasValidPngIEND(trailer) ? trailer.byteLength : 0;
}

function parseJpeg(jpegData) {
    const uint8Array = new Uint8Array(jpegData);
    if (!uint8Array.slice(0, JPEG_MAGIC.length).every((v, i) => v == JPEG_MAGIC[i]))
...
function parseWebp(webpData) {
    const uint8Array = new Uint8Array(webpData);
    if (!uint8Array.slice(0, WEBP_MAGIC.length).every((v, i) => v == WEBP_MAGIC[i]))
...
function parseAvif(avifData) {
    if (indexOfSequence(new Uint8Array(avifData), AVIF_MAGIC) !== 4) throw new Error("Not an AVIF file!");
