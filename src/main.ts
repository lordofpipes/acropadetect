const switchTheme = document.getElementById("theme-switcher") as HTMLDivElement;
const lightMode = switchTheme.querySelector(".light-mode") as HTMLDivElement;
const darkMode = switchTheme.querySelector(".dark-mode") as HTMLDivElement;

let isLight = !(window.matchMedia && window.matchMedia("(prefers-color-scheme: dark)").matches);

document.addEventListener("DOMContentLoaded", () => {
    document.documentElement.setAttribute("data-theme", isLight ? "light" : "dark");
    (switchTheme.querySelector(".os-default") as HTMLDivElement).style.display = "none";
...
const workerCountPicker = document.getElementById("threads-picker") as HTMLInputElement;
const thresholdPicker = document.getElementById("threshold-picker") as HTMLInputElement;
const resultsFooter = document.getElementById("results-footer") as HTMLDivElement;
const progressBar = resultsFooter.querySelector(".progress") as HTMLProgressElement;
const currentlyProcessing = resultsFooter.querySelector(".currently-processing") as HTMLDivElement;
const resultSummary = document.getElementById("output") as HTMLDivElement;
const resultText = resultSummary.querySelector(".output-text") as HTMLParagraphElement;
const resultCode = resultSummary.querySelector(".output-code") as HTMLPreElement;
const resultPanel = document.getElementById("images") as HTMLDivElement;
            ...
            switch (data.type) {
                ...
                case "result":
                    if (data.type === "error" || data.result >= threshold) {
                        ...
                        if (data.type === "error")
                            header.appendChild(
                                document.createTextNode(`Unknown result. File parse error: ${data.error}`),
                            );
                        else
                            header.appendChild(
                                document.createTextNode(
                                    `Vulnerable! Has ${(data.result / 1024).toFixed(1)} KiB more than it should`,
                                ),
                            );
